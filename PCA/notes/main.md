...
of plots (where $N$ is the number of features).  The  entry with $(i,j)$ coordinates in the grid is a scatter plot of the $i^{th}$
...
If $u^{\intercal}Du\ge 0$ for all $u\in\mathbf{R}^{N}$, then all eigenvalues $\lambda_{i}$ are non-negative.  In the latter case we say that $D$ is *positive semi-definite.*
...
**Theorem:** The solution $w_{1},\ldots, w_{s}$ to this constrained optimization problem  is $u_{1},\ldots, u_{s}$ where $u_{i}$ is the $i^{th}$
