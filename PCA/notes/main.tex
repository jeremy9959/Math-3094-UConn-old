...
\hypertarget{sec:symmmat}{%
\paragraph{Eigenvalues and eigenvectors of symmetric
matrices}\label{sec:symmmat}}
...
\tightlist
\item
  All of the eigenvalues \(\lambda_{1},\ldots, \lambda_{N}\) of \(D\)
  are real.
\item
  If \(u^{\intercal}Du\ge 0\) for all \(u\in\mathbf{R}^{N}\), then all
  eigenvalues \(\lambda_{i}\) are non-negative.
\item
  If \(v\) is an eigenvector for \(D\) with eigenvalue \(\lambda\), and
...
\item
  ...
  \(\mathbf{R}^{N}\) made up of eigenvectors of \(D\) corresponding to
...
If we combine this theorem with our result from \cref{sec:symmmat} then
we get a complete picture. Let \(D_{0}\) be the covariance matrix of our
data. Since \[
